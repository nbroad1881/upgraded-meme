...
class SaveCallback(TrainerCallback):
    def __init__(
        self, score_threshold, metric_name, weights_only=True, lower_is_better=False
    ) -> None:
        """
        After evaluation, if the `metric_name` value is higher than
        `score_threshold` the model will get saved.
        If `metric_name` value is better than `score_threshold`, then
        `metric_name` value becomes the new `score_threshold`.

        if lower_is_better is True, better means `metric_name` < `score_threshold`
    ...
    def on_evaluate(
        ...
        **kwargs,
        ...
        if self.lower_is_better:
            has_improved = metric_value < self.score_threshold
        else:
            has_improved = metric_value > self.score_threshold
        if has_improved:
            logger.info(f"Saving model.")
            self.score_threshold = metric_value
